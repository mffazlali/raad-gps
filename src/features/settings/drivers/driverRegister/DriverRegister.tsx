...
import {useDriver, useCreateDriver, useUpdateDriver} from '../../../../common/serverStore/useDriver.ts'

const DriverRegister = ({setOpen}: {setOpen: React.Dispatch<React.SetStateAction<boolean>>}) => {
  ...
  const { data: driver, isLoading: isLoadingDriver } = useDriver(id ? parseInt(id) : undefined)
  ...
  const handleSave = useCatch(async (values: any, actions: FormikHelpers<any>) => {
    ...
    try {
      if (id) {
        await updateDriver.mutateAsync({ id: parseInt(id), driver: values })
      } else {
  ...
  const {form} = useForm({
    formGroup: {
      ...
      'uniqueId': {
        value: '',
        validations: [{required: 'وارد کردن شناسه الزامی است'}],
      },
    },
        ...
        >
          <div id={'inputsWrapper'} className={cls(styles.inputsWrapper)}>
            <Card title={'ضروری'} contentClassName={styles.inputs}>
              ...
              <div className={styles.input}>
                <KitInputWrapper label="شناسه" required={true} name="uniqueId" direction={'col'}
                                 loading={loadingForm}>
