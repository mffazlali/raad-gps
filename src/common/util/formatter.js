...
export const formatState = (value, t) =>
  value ? t('sharedInActive') : t('sharedActive')
...
export const formatTime = (value, format, hours12, isShamsi = true) => {
  if (value) {
    let d = moment(value, 'jYYYY-jMM-jDD HH:mm:ss')
    if (!isShamsi) {
      d = dayjs(value)
      switch (format) {
        case 'date':
        ...
        default:
          return d.format(
            hours12 ? 'YYYY-MM-DD hh:mm:ss A' : 'YYYY-MM-jDD HH:mm:ss',
          )
      }
    } else {
      switch (format) {
        ...
        case 'dateRaw':
          return d.format('jYYYYjMMjDDHHmmss')
        case 'time':
          return d.format(hours12 ? 'hh:mm:ss A' : 'HH:mm:ss')
        case 'minutes':
          return d.format(hours12 ? 'jYYYY-jMM-jDD hh:mm A' : 'jYYYY-jMM-jDD HH:mm')
        default:
          return d.format(
            hours12 ? 'jYYYY-jMM-jDD hh:mm:ss A' : 'jYYYY-jMM-jDD HH:mm:ss',
          )
      }
...
export const formatNotificationTitle = (t, notification, includeId = null) => {
