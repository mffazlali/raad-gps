...
import { devicesActions } from '../clientStore'
import { useCatch, useCatchCallback } from '../util/reactHelper.js'
...
const StatusCard = ({
  ...
  const handleRemove = useCatch(async (removed) => {
    if (removed) {
      try {
        const response = await axiosInstance.get(`/api/devices/${currentUser.id}`)
  ...
  const handleGeofence = useCatchCallback(async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const permissionResponse = await axiosInstance.post('/api/permissions', {
        ...
        if (permissionResponse.status === 200) {
          ...
          throw Error(permissionResponse.data)
        }
      }
    } catch (error) {
      throw Error(error.response?.data || error.message)
