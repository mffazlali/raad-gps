...
import {Element, Runner, SVG, Text, Timeline} from '@svgdotjs/svg.js'
import {map} from './MapView.jsx'
import {mapIcons, mapImageIcons, mapImages} from './preloadImages.js'
...
export const prepareImage = (imageUrl) => {
  ...
  return context.getImageData(0, 0, imageUrl.width, imageUrl.height)
}
...
export const geofenceToFeature = (theme, item) => {
  let geometry
  if (item && item.area && item.area.indexOf('CIRCLE') > -1) {
...
export const addSvgIcon = async (category, color) => {
  const name = `${category}${color ? color : ''}`
  if (!map.getImage(name)) {
    const background = await loadImage(backgroundSvg)
    const icon = await loadImage(mapImageIcons[category])
    const imageData = prepareIcon(background, icon, color)
    if (!map.getImage(name)) {
  ...
  // return loadImage(category).then((icon) => {
  //   return prepareIcon(
  //     background,
  //     icon,
  //     color,
